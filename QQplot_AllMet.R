...
umethods<-c("burden", "MB" , "SKAT_", "VT_", "singlevar")
print(umethods[m])
...
PathF<-paste0("/lustre/scratch115/realdata/mdt0/projects/int_wes_metabol/raremetal/AllMetabolites/raremetal_inverse_norm_AllMet/",Approaches[a],"/")

Filein<-list.files(path = paste0(PathF,"AllMerged","/"), full.names = F,  recursive = F, pattern =paste0("Combined_ALL_INTERVAL_QCv1.0_inverse_norm_",Approaches[a],".*.",traits[1],".meta.*.results"))
ptmA <- proc.time()
...
for (mf in 1:length(maf)){
  ...
  for (i in 1:length(traits)){
    #i=1
    print(traits[i])
    fn <- paste(PathF,"AllMerged","/Combined_ALL_INTERVAL_QCv1.0_inverse_norm_",Approaches[a],".",maf[mf],".",traits[i],".meta.",umethods[m],".results", sep="")
