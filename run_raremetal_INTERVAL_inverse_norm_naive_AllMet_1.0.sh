...
raremetal=/lustre/scratch115/projects/int_wes_metabol/raremetal/AllMetabolites/raremetal_inverse_norm_AllMet/naive
...
do
	echo $MAF
	for group in `ls /lustre/scratch115/projects/int_wes_metabol/VEP_85_GENCODE24_GFF3/Chr*/naive/*${MAF}.list`
	do 
    		...
    		echo "raremetal --summaryFiles $score --covFiles $cov --groupFile $group --maf 0.001 --SKAT --burden --longOutput --tabulateHits --prefix Chr${chr}_INTERVAL_QCv1.0_naive.${trait}"
    		${rareMet_S} --summaryFiles $score --covFiles $cov --groupFile $group --maf ${MAF} --SKAT --burden --MB --VT  --longOutput --hwe 1e-06 --tabulateHits --hitsCutoff 1e-04  --prefix Chr${chr}_INTERVAL_QCv1.0_inverse_norm_naive.MAF${MAF}.${trait}
	done
done
